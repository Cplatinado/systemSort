...
@section('content_header')
    <h3>Promoções {{ ($ativos == 'home' ? '' : $ativos ) }}</h3>
...
@section('content')
    ...
    <div class="row ">
        <div class="col-sm-12">
            <h5>Ações</h5>
            <a href="{{route('admin.promocoes.index')}}" class="btn btn-sm  {{($ativos == 'ativos' ? 'home' : '')}} bg-gradient-success mr-2  "><i
                    class="fas fa-list-ul"></i> Todas Promoções </a>
            <a href="{{route('admin.promocoes.ativos')}}" class="btn btn-sm  {{($ativos == 'ativos' ? 'active' : '')}} bg-gradient-success mr-2  "><i
                    class="fas fa-list-ul"></i> Ativos </a>

            <a href="{{route('admin.promocoes.inativos')}}" class="btn btn-sm {{($ativos == 'inativos' ? 'active' : '')}} bg-gradient-warning mr-2" ><i
                    class="fas fa-list-ul"></i> Inativos</a>

            <a href="" class="btn btn-sm bg-gradient-primary  " data-toggle="modal" data-target="#add"><i
                    class="fas fa-plus"></i> Nova Promoção</a>
    ...
    <hr>
    <h4>Ultimas Promoções {{ ($ativos == 'home' ? '' : $ativos ) }}</h4>

    <div class="row">
        @foreach($prometions as $prometion)

            <div class="col-md-3">
                <div class="card card-info">
                    <div class="card-header">
                        <h3 class="card-title">{{$prometion->title}}</h3>
                    </div>
                    <div class="card-body ">
                        <img width="100%" src="{{$prometion->cover()}}">
                    </div>

                    <div class="card-footer">
                        <a href="{{route('admin.promocoes.edit',['promoco'=>$prometion->id])}}" class="btn btn-block bg-gradient-success"> <i class="fas fa-plus"> </i> Ver mais</a>
    ...
    <div class="modal fade" id="add" style="display: none;" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{route('admin.promocoes.store')}}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-header">

                        <h4 class="modal-title"><i class="fas fa-gift"></i> Adcionar Promoção</h4>
                        <hr>
                    ...
                    <div class="modal-body">
                            ...
                            <div class="col-sm-12">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="titulo" required class="form-control"
                                           placeholder="Ex: Promoção Do Mês" value="{{ old('titulo') }}">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    <label><i class="fas fa-filter"></i>Data Da Promoção</label>
